...
- 适配 HBuilderX 5.26 的 UTS 数值类型：相机录入性能模式显式转换为 Android `Int`。
- 相机录入结果监听改为单任务管理，并在收到结果后立即反注册，避免重复回调与页面引用泄漏。
- 全屏人脸搜索将调用方传入的超时时间同步应用到 SDK 搜索流程，默认超时调整为 2000ms。
- 最低 Android 版本统一为 API 23（Android 6）。

## 4.0.0（2026-09-09）
- 人脸搜索，人脸抓拍支持uniapp ,uniappX 
- 支持兼容模式，标准模式组件以及UTS API插件
- 细节调整以便更多种方式接入
## 3.0.0（2026-09-09）
- 人脸搜索，人脸抓拍支持uniapp ,uniappX 
- 支持兼容模式，标准模式组件以及UTS API插件
