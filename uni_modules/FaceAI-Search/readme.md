...
| 项目与页面类型 | 能否内嵌人脸搜索/抓拍原生组件 | 接入方式 |
| --- | :---: | --- |
| 传统 uni-app Vue3 的普通 `.vue` 页面 | 不能直接内嵌 | 使用全屏 UTS API；需要页面内嵌时，跳转到 `.nvue` 页面 |
| 传统 uni-app Vue3 的 `.nvue` 页面 | 可以 | 使用兼容模式原生组件 |
| uni-app x 的 `.uvue` 页面 | 可以 | 推荐使用标准模式组件；需要兼容旧组件时也可使用兼容模式，不要求内嵌时可使用全屏 UTS API |

> 本插件当前仅支持 Android。标准模式组件用于 uni-app x；传统 uni-app 普通 `.vue` 页面不能直接承载 Android 原生 View。
